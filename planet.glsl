...
uniform sampler2DShadow DirShadowMap;                // The depth map generated by the engine
uniform mat4            LightSpaceMatrix[MAX_LIGHTS]; // Matrix to transform fragment to light space
...
// Vertex shader output
out vec4 fPosLightSpace[MAX_LIGHTS];
...
for(int i = 0; i < NLights; i++) {
        fPosLightSpace[i] = LightSpaceMatrix[i] * vec4(fPosition.xyz, 1.0);
    }
...
void main()
{
    ...
    for (int i=0; i<NLights; i++)
    {
		...
		// Donatelo200 Night time culling
		//float Angle = -acos((Radiuses.y)/(Radiuses.y + Radiuses.z*3.14));

		//float rayleighFade = smoothstep(Angle, -Angle, EyeMuS);
...
const float Ring_Shine = 1;

if (Ring_Shine == 1){
        #ifdef RINGS
            // Determine the orientation of the ring plane
            float ringNormalL = (SurfParams3.x == 0.0) ? lightVec.y : lightVec.z;
            float ringNormalN = (SurfParams3.x == 0.0) ? Normal.y : Normal.z;  // latitude
			//float ringNormalEW = (SurfParams3.x == 0.0) ? Normal.x : Normal.z;  //longitude
            
            // Check if the sun and the surface fragment are on the same side of the ring plane
            float sameSide = step(0.0, ringNormalL * ringNormalN);
            
            //  Rings horizon
            float latSine = abs(ringNormalN);
            //float lonSine = abs(ringNormalEW);
			
			float InnerRing = Radiuses.y / (RingsParams.x);  // Inner ring rel to planet
			float OuterRing = Radiuses.y / ((RingsParams.x + (1/RingsParams.w))); // Outer ring rel to planet
			float MeanRing = Radiuses.y / (Radiuses.y + (RingsParams.x + (1/RingsParams.w))/2); // mean ring rel to planet
            ...
            float horizonStart = sqrt(acos(InnerRing)/acos(0)); // Latitude where inner ring dips below horizon
            float horizonEnd   = sqrt(acos(OuterRing)/acos(0)); // Latitude where outer ring dips below horizon
    ...
    vec2 lonNormal = (SurfParams3.x == 0.0) ? Normal.xz : Normal.xy;
    vec2 lonLight  = (SurfParams3.x == 0.0) ? lightVec.xz : lightVec.xy;
    
    // Normalize Longitude dir
    lonNormal = normalize(lonNormal + 1e-6);
    lonLight  = normalize(lonLight + 1e-6);
    ...
    float nightShadow = min(1, mix(MinIllum, 1, midnightFade));   //abs(ringNormalL*ringNormalL*ringNormalL)* horizonVisibility + MinRing
    // ----------------------------------------------
			
			
             float Bright = 1-latSine/(horizonEnd); 
            
            // Intensity depends on how much ring area is lit, visible, AND above the horizon
            float ringIntensity = abs(ringNormalL) * latSine  * horizonVisibility * nightShadow;   //abs(ringNormalL) * latSine  * horizonVisibility * nightShadow;

// ==============================================
            // NEW: Dim ringshine when an eclipse crosses the rings
            #ifdef ECL
                // 1. Calculate a representative "bright point" on the rings (sub-solar point on the ring plane)
                ...
                // 2. Find the object-space coordinate of the rings at the mean radius
                float ringRadiusOS = (RingsParams.x + 1.0 / RingsParams.w) / EyePosLocal.w;
                vec3 ringPos = ringSunDir * ringRadiusOS;
                
                // 3. Evaluate the eclipse shadow exactly at this point in the rings
                ...
                // 4. Dim the ringshine based on how much of the ring system is eclipsed
                ringIntensity *= (1.0 - ringEclipse);
                
                // 5. Also dim ringshine if the surface fragment itself is inside an eclipse.
                // This prevents eclipse shadows on the ground from being washed out by glowing rings.
                //ringIntensity *= EclipseMask; 
            #endif
            // ==============================================
            ...
            // Scale by ring density (RingsParams.z) to ensure wispy rings cast less light
            vec3 ringShine = LightColor[i].rgb * ringIllum * RingsParams.z;
            
			
			
			
			/* Add ringshine to the ambient terrain and sea terms, attempt spec reflection.  (Broken not usable yet)
            ambTerTerm += ringShine * aoTer; 
            ambSeaTerm += ringShine;
            
            #ifdef AWATER
            float ringFresnel = 0.02 + 0.98 * pow(1.0 - abs(eyeVecTS.z), 4.0);
            specSeaAccum += ringShine * ringFresnel;
            
            waterFogAccum += ringShine;
            #endif
			*/
			
			
			
            // Add ringshine to the ambient terrain and sea terms old (working fine/better)
            ambTerTerm += ringShine * aoTer;
            ambSeaTerm += ringShine;
			#ifdef AWATER
            waterFogAccum += ringShine;
            ...
            // Atmospheric Haze
                ...
                EyeMuS = horizonVisibility;  //1-latSine/(horizonEnd); 
                
                // Ambient air on terrain
                if (atmoHorFix)
                    Inscatter += inscatterGroundFix(vec3(0.0)) * ringShine;
                else
                    Inscatter += inscatterGround(vec3(0.0)) * ringShine;
