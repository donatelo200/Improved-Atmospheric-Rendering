...
void main()
{
    ...
    for (int i=0; i<NLights; i++)
    {
        ...
        #ifdef RINGS
        {
            ...
            float delta_visible = acos(clamp(horizon_cos / max(normalRingPlaneLen, -1.0), -1.0, 1.0));
            ...
            float overlap = max(0.15, min(delta_repr + ov_b, delta_shadow) - max(delta_repr - ov_b, -1.0));
            ...
            // Eclipse from other moons/planets
              #ifdef ECL
                vec3 dir_side_ring = cross(poleVec, dir_sun_ring); // fixed basis, independent of the fragment
